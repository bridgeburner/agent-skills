# Canvas 2D Scene Patterns — 3D Effects Reference

Zero-dependency 3D effects using Canvas 2D with manual projection. Every pattern is complete, copy-pasteable, and runs inline in a single HTML file. Use only when the user opts in during Phase 2 (Step 2.1b). Apply to 2-3 slides per deck max. See the [decision framework](../SKILL.md#optional-3d-enhancements) for per-slide guidance. These are all Tier 2 (Canvas 2D). Tier 1 (CSS 3D) is standard `perspective` + `translateZ`. Tier 3 (Three.js CDN) is explicit user opt-in only.
...
**Pairs with:** Any preset, especially dark themes (Bold Signal, Neon Cyber, Dark Botanical)
...
function animate() {
  ...
  for (const p of particles) {
    ...
    ctx.fillStyle = `rgba({accentColor}, ${alpha})`;  // e.g. "120, 180, 255"
    ctx.fill();
  }
...
## Pattern: Rotating Wireframe

**Lines:** ~70 | **Impact:** 7/10 | **Reliability:** 8/10
**Pairs with:** Neon Cyber, Futuristic Engineering, Terminal Green
...
function animate() {
  ...
  for (const [i, j] of edges) {
    ...
    ctx.strokeStyle = `rgba({accentColor}, ${alpha})`;  // e.g. "0, 255, 200"
...
**Pairs with:** Any dark theme (Neon Cyber, Dark Botanical, Bold Signal)
...
**Pairs with:** Bold Signal, Swiss Modern, Futuristic Engineering
**Fallback:** AI-generated globe image (create-image)
...
function animate() {
  ...
  for (const p of sorted) {
    ...
    ctx.fillStyle = p.highlight
      ...
      : `rgba({accentColor}, ${alpha * 0.5})`;     // e.g. "150, 200, 255"
    ctx.fill();
  }
  animId = requestAnimationFrame(animate);
}
...
## Pattern: Connected Network Graph

**Lines:** ~60 | **Impact:** 7/10 | **Reliability:** 7/10
**Pairs with:** Futuristic Engineering, Neon Cyber
...
## Pattern: Abstract Mesh Background

**Lines:** ~90 | **Impact:** 8/10 | **Reliability:** 7/10
**Pairs with:** Dark Botanical, Dark Fantasy, Steampunk
**Fallback:** CSS gradient or AI-generated abstract image (create-image)
...
function throttledAnimate() {
  if (++frameCount % 2 !== 0) { /* run actual draw logic */ }
